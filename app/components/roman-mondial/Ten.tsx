import React from "react";

type Props = {};

const Ten = (props: Props) => {
  return (
    <div>
      <div className="nine bg-[#f2f2f2] dark:bg-background p-2 rounded-md">
        <p className="fr">
          Thérèse allait avoir dix-huit ans. Un jour, seize années auparavant,
          lorsque madame Raquin était encore mercière, son frère, le capitaine
          Degans, lui apporta une petite fille dans ses bras. Il arrivait
          d’Algérie.
        </p>
        <p className="ar">
          تيريز على وشك بلوغ الثامنة عشرة من العمر. في يوم من الأيام، قبل ستة
          عشر عامًا، وعندما كانت السيدة راكان لا تزال تعمل كبائعة خيوط، جاء
          شقيقها، الكابتن ديجان، يحمل طفلة صغيرة في ذراعيه. كان قد وصل من
          الجزائر.
        </p>
        <p className="fr">
          – Voici une enfant dont tu es la tante, lui dit-il avec un sourire. Sa
          mère est morte... Moi je ne sais qu’en faire. Je te la donne.
        </p>
        <p className="ar">
          &#39;هذه الطفلة هي ابنة أختك&#39;، قال لها بابتسامة. &#39;والدتها ماتت
          ... وأنا لا أعرف ماذا أفعل بها. أنا أعطيك إياها.&#39;
        </p>
        <p className="fr">
          La mercière prit l’enfant, lui sourit, baisa ses joues roses. Degans
          resta huit jours à Vernon. Sa soeur l’interrogea à peine sur cette
          fille qu’il lui donnait. Elle sut vaguement que la chère petite était
          née à Oran et qu’elle avait pour mère une femme indigène d’une grande
          beauté. Le capitaine, une heure avant son départ, lui remit un acte de
          naissance dans lequel Thérèse, reconnue par lui, portait son nom. Il
          partit, et on ne le revit plus ; quelques années plus tard, il se fit
          tuer en Afrique.
        </p>
        <p className="ar">
          استلمت البائعة الطفلة وابتسمت لها، وقبلت خديها الزهريين. بقي ديجان
          ثمانية أيام في فيرنون، وسألته شقيقته بالكاد عن الفتاة التي كان يعطيها
          إياها. عرفت بشكل غامض أن الطفلة الغالية ولدت في وهران وأن والدتها كانت
          امرأة أصلية من جمال رائع. قبل ساعة من مغادرته، سلم الكابتن لشقيقته
          شهادة ميلاد تحمل اسم تيريز وكان قد اعترف بها. رحل ولم يعد يظهر مرة
          أخرى. وبعد بضع سنوات، قتل في أفريقيا.
        </p>
        <p className="fr">
          Thérèse grandit, couchée dans le même lit que Camille, sous les tièdes
          tendresses de sa tante. Elle était d’une santé de fer, et elle fut
          soignée comme une enfant chétive, partageant les médicaments que
          prenait son cousin, tenue dans l’air chaud de la chambre occupée par
          le petit malade. Pendant des heures, elle restait accroupie devant le
          feu, pensive, regardant les flammes en face, sans baisser les
          paupières. Cette vie forcée de convalescente la replia sur elle-même ;
          elle prit l’habitude de parler à voix basse, de marcher sans faire de
          bruit, de rester muette et immobile sur une chaise, les yeux ouverts,
          et vides de regards.
        </p>
        <p className="ar">
          تيريز نشأت وهي تنام في نفس السرير الذي ينام فيه كاميل، تحت الأحضان
          الدافئة لعمتها. كانت بصحة جيدة، ولكنها تعاملت كأنها طفلة مريضة، تشارك
          الأدوية التي يتناولها ابن عمها، وتبقى في الهواء الدافئ في الغرفة التي
          يمكث فيها المريض الصغير. لساعات طويلة، كانت تجلس متربعة أمام النار،
          متأملة، تنظر إلى النيران بدون أن تحرك جفونها. هذه الحياة القسرية
          كمريضة جعلتها تنغلق على نفسها؛ اعتادت التحدث بصوت خافت، والمشي بدون
          إصدار أي صوت، والبقاء صامتة وساكنة على كرسي، عيناها مفتوحة وخالية من
          أي نظرة.
        </p>
      </div>
    </div>
  );
};

export default Ten;
